...
rpcPassword=$(head /dev/urandom | tr -dc A-Z0-9 | head -c 12 ; echo '') && echo ${rpcPassword} > password

sleep 2

PUBKEY=$(./openethereum account new --keys-path=data/keys --password=password --chain=genesis.json)
...
cd ~/root/etl-val-node
echo "Save Info Below"
echo "${PUBKEY}"
echo "${rpcPassword}"
echo "${MONIP}" 

echo "Chain downloading..."
curl -L "https://etherlite.ams3.digitaloceanspaces.com/chains5798.zip" -o chains.zip
echo "Chain Unzip..."
unzip chains.zip
